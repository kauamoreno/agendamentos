...
class NotificacaoCustomizada {
  ...
  final String?
      payload; // caso houver necessidade de mandar outros tipos de dados, a rota que vai ser acessada pelo usuario vai ser tambem pelo payload, ao ser clicada

  NotificacaoCustomizada(
      {required this.id,
      required,
      required this.title,
      required this.body,
...
class NotificacaoEservicos {
  late FlutterLocalNotificationsPlugin
      localNotificationsPlugin; //inicialização tardia, volta um mesmo valor sempre
  late AndroidNotificationDetails
      androidDetalhes; // detalhes de notificação para o android

  NotificacaoEservicos() {
    localNotificationsPlugin =
        FlutterLocalNotificationsPlugin(); //instancia para acessar os dados locais do sistema operacional
    _setupNotificacoes();
  }
  ...
  Future<void> _setupTimezone() async {
    ...
    final String? timezone = await FlutterNativeTimezone
        .getLocalTimezone(); //neste trecho vamos pegar o timeZone do sistema operacional que está setado
  ...
  _onNotificacaoSelecionada(String? payload) {
    if (payload != null && payload.isNotEmpty) {
      Navigator.of(Routes.navegadorChave!.currentContext!)
          .pushReplacementNamed(payload);
    }
  }

  //para enviar notificação para o usuario
  mostrarNotificacao(NotificacaoCustomizada notificacaoCustomizada) {
    androidDetalhes = const AndroidNotificationDetails('Notifições', "Lembrar",
        importance: Importance.max,
        priority: Priority.max,
        enableVibration: true);

    localNotificationsPlugin.show(
        notificacaoCustomizada.id,
        notificacaoCustomizada.title,
        notificacaoCustomizada.body,
        NotificationDetails(android: androidDetalhes),
        payload: notificacaoCustomizada.payload);
  }
}
